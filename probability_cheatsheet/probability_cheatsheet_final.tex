...
\geometry{top=.15in,left=.2in,right=.2in,bottom=.2in,includehead}
...
\subsection{Universality of Uniform (UoU)} When you plug any CRV into its own CDF, you get a $\Unif(0,1)$ random variable. When you plug a $\Unif(0,1)$ r.v.~into an inverse CDF, you get an r.v.~with that CDF. For example, let's say that a random variable $X$ has CDF
...
\section{Discrete Distributions}\hrule height 1pt \smallskip
...
\section{Multivariate Distributions} \hrule height 1pt \smallskip
...
\section{Distribution Properties} \hrule height 1pt \smallskip
...
\section{Moments and MGFs}\hrule height 1pt \smallskip
...
\section{Joint PDFs and CDFs}\hrule height 1pt \smallskip
...
\section{Markov Chains}\hrule height 1pt \smallskip
...
\textbf{Columns Summing to One Implies Stationarity} If each column of the transition matrix $Q$ sums to 1, then the uniform distribution over all the states, $(1/M,1/M,\dots,1/M)$ is a stationary distribution. One example of this is a \underline{symmetric transition matrix} (in which case it's also reversible).
...
\section{Likelihood}
Let $f_{\vec Y}$ be a model for the joint density function of all the observations. Let $\vec{y}$ be the observed value of $\vec{Y}$. Then the likelihood function is:
\[ L(\theta;\vec y)=f_{\vec{Y}}(y|\theta) \]
It is regarded as a function of $\theta$, with $y$ treated as fixed.
\begin{description}
\item[Frequentist Interpretation] $\theta$ is regarded as fixed but unknown, and it does not have a distribution. If $L(\theta_2)>L(\theta_1)$, strictly speaking we cannot say that $\theta_2$ is more likely than $\theta_1$ to be the true value of $\theta$. However, we can say that $\theta$ seems more plausible than $\theta$ as a source for generating the data.
...
\end{description}
\section{Statistics, Estimands, Estimators, \& Estimates}
\begin{description}
...
The \textbf{maximum likelihood estimate} of $\theta$ is the value $\hat\theta$ that maximizes the likelihood function $L(\theta; y)$. The corresponding estimator is called the \textbf{maximum likelihood estimator}. That is, if the maximum likelihood estimate is $T(y)$, then the maximum likelihood estimator is $T(Y)$.
\begin{description}
...
\end{description}
\section{Bias, Variance and Loss Functions}
\begin{description}
...
\end{description}
\section{Estimators for CDF and PDF}
...
where $h>0$ is called the \textit{bandwidth} and $K$ is called the \textit{kernel function}. The kernel function must be a PDF (nonnegative and summing to 1).

For instance, the Gaussian kernel takes $K$ to be a Normal PDF centered at 0. The intuitive idea of the KDE for the Gaussian case is to place a little Normal distribution centered at each data point, and then mix all these Normal distributions together to obtain a single smooth PDF. The bandwidth
...
\subsection{Definition} An r.v. follows NEF if its PDF is in the form $$f_y(y|\theta) = e^{\theta y - \Psi(\theta)}h(y)$$ Where $\theta$ is the natural parameter. Note that $\theta$ doesn't necessarily have to be a parameter of interest, e.g. it could be $-\mu$ instead of $\mu$ for a normal distribution.
...
\section{MLEs \& Fisher Informations}
\begin{itemize}
